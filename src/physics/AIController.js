...
export class AIController {
  constructor(arcadeCar, spline, topSpeed, weaponsManager = null) {
    this.car = arcadeCar;
    this.spline = spline;
  ...
  update(delta, playerCar, allCars) {
    ...
    // 2. Target waypoint in world space
    ...
    if (turnSeverity > 0.45) {
      this.input.throttle = 0.45; // Slow down for corners
    } else if (turnSeverity > 0.8) {
      this.input.throttle = -0.2; // Hard brake into tight hairpins
    } else {
